...
Status: planned reference integration; a small ingestion planner is implemented, but no webhook server, Paperless API client or exporter is complete.
...
The current `integrations/paperless/agent` package provides deterministic planning for the ingestion side of this flow. It maps authenticated source events to explicit `original` or `archive` representations, tenant/source/version identity and a stable idempotency key. It also ignores events that only touch configured SigmaProof evidence fields so status write-back cannot loop into new evidence. Byte retrieval, webhook authentication, periodic reconciliation, durable export and write-back are still future work.
